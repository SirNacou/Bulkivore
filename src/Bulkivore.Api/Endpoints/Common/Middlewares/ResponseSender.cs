...
namespace Bulkivore.Api.Endpoints.Common.Middlewares;
...
public class ResponseSender : IGlobalPostProcessor
{
    public Task PostProcessAsync(IPostProcessorContext ctx, CancellationToken ct)
    {
        ...
        if (!errorOr.IsError) return ctx.HttpContext.Response.SendAsync(GetValueFromErrorOr(errorOr), cancellation: ct);
